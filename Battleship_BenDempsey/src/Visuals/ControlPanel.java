 ...
 * Visual control panel that appears on the top right quadrant of the GUI in battleship
 */
public class ControlPanel extends JComponent {
    ...
    protected void paintComponent(Graphics g){
        ...
        g2d.setColor(Color.GRAY);//draw background
        ...
        g2d.drawString("Opponent Remaining Ships:", 5*Y/7, (int)((5*Y/14) * (600./Y)));//write red text
        g2d.scale(700./X,600./Y);
        g2d.setColor(Color.BLACK);
        g2d.fillRect(3*X/8 - (X/50), 3*Y/7 - (Y/30), 5*X/8, 4*Y/7 );//draw background of ships
        g2d.setColor(Color.white);
        for(int i =0; i <= 5; i++)
        g2d.drawLine(3*X/8 - (X/50), (9+2*i)*Y/21, X-(X/50), (9+2*i)*Y/21);//draw white reference lines

        g2d.setColor(Color.BLACK);
        g2d.drawImage(aircraftcarrier, 3*X/8, 3*Y/7, null);//draw ship
        if(!board.hasShip(8) && !board.hasShip(9)) {//if sunk then cross through it with a red line
            g2d.setColor(Color.red);
            g2d.drawLine(3 * X / 8, 3 * Y / 7, 3 * X / 8 + X / 10, 3 * Y / 7 + Y / 2);
            g2d.setColor(Color.BLACK);
        }
        g2d.drawImage(battleship, 4*X/8, 3*Y/7, null);
        if(!board.hasShip(6) && !board.hasShip(7)) {//if sunk draw red
            g2d.setColor(Color.red);
            g2d.drawLine(4 * X / 8, 3 * Y / 7, 4 * X / 8 + X / 15, 3 * Y / 7 + 2*Y /5);
            g2d.setColor(Color.BLACK);
        }
        g2d.drawImage(destroyer, 5*X/8, 3*Y/7, null);
        if(!board.hasShip(4) && !board.hasShip(5)) {//if sunk draw red
            g2d.setColor(Color.red);
        ...
        if(!board.hasShip(10) && !board.hasShip(11)){//if sunk draw red
            g2d.setColor(Color.red);
        ...
        if(!board.hasShip(2) && !board.hasShip(3)) {//if sunk draw red
            g2d.setColor(Color.red);
        ...
        g2d.scale(X/400., 3*Y/400.);
        g2d.drawString(rowOneText, 10, 40);//write top text
        g2d.scale(fontScalerX, fontScalerY);
        g2d.drawString(rowTwoText, 10, 40 + ((X*Y)/(X+Y))/10); //write bottom text
